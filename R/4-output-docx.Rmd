...
editOutputTablebyToxicity <- function(input_df, output_df, target_toxicity, n){
  df_by_toxicity <- subset(input_df, toxicity == target_toxicity)
  output_row <- c(target_toxicity, n)
  output_colnames_1 <- c('Toxicity', 'N')
  for (i in 1:nrow(df_by_toxicity)){
    output_row <- c(output_row, df_by_toxicity[i, 'count'], df_by_toxicity[i, 'percent'])
    if (nrow(output_df) == 0){
      output_colnames_1 <- c(output_colnames_1, paste0('Grade', df_by_toxicity[i, 'grade'], '(N)'), paste0('Grade', df_by_toxicity[i, 'grade'], '(%)'))
    }
  }
  if (nrow(output_df) == 0){
...
for (i in 1:length(target_visit_list)){
  ...
  n <- unique(target_visit$n)
  output_df <- data.frame()
  target_toxicity_list <- unique(target_visit$toxicity)
  ...
  print(kable(output_df, caption=paste0('Phase:', target_visit_list[i]), row.names=F))
}
```
